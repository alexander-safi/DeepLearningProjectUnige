...
# The below is a version with the auxiliary loss 
# 
...
# Auxilliary Loss
# Function NOT USED
# Written for organisation purpose
# Directly used in the next cell
def auxloss(model):
    
    lmbda = 0.6
    
    loss0 = criterion(output0, train_target[i].to(torch.float32))
    loss1 = criterion(output1, train_target[i].to(torch.float32))
    loss2 = criterion(output2, train_target[i].to(torch.float32))
    
    auxloss = loss0 + lmbda * (loss1 + loss2)
    loss.backward()
    optimizer.step()
...
for epoch in range(25):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(train_input):
        ...
        auxloss = loss + lmbda * (loss1 + loss2)
        ...
        runninglossAUX += auxloss.item()
        
        if i % 900 == 0:    # print every 2000 mini-batches
            print(f'[{epoch + 1}, {i + 1:5d}] loss: {running_loss / 2000:.3f}')
            
            print(f'[{epoch + 1}, {i + 1:5d}] loss: {runninglossAUX / 2000:.3f}')
            running_loss = 0.0
            runninglossAUX = 0.0
